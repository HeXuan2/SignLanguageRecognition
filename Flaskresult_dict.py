...
def result_dic(rec_input_size,rec_data):
    ...
    model_save_name = opt.model_name + "_output" + str(output_size) + "_input" + str(time_step) + "x" + str(
        input_size) + ".pkl"
    ...
    if rec_input_size != input_size:
        ...
        # print(class_dict, pre_class, dict_start_index)
        pre_class_name = class_index2name(class_dict, pre_class, dict_start_index)  # 函数根据类别索引将预测的类别转换为类别名称

        print(pre_prob, pre_class_name)
